...
export default function CodeEditor({ filePath, content, onChange }: CodeEditorProps) {
	...
	const getBinaryMessage = () => {
		if (isImageFile) return '这是图片（二进制数据），内容过大无法显示。'
		if (isFontFile) return '这是字体文件（二进制数据），内容过大无法显示。'
		return ''
	}
		...
		if (!highlighterRef.current) {
			...
			const initHighlighter = async () => {
				try {
					...
					const response = await fetch(`${import.meta.env.BASE_URL}textmate-sintax/typst.tmLanguage.json`)
